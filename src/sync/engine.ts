  ...
  for (const op of ops) {
    try {
      if (op.op === 'upsert') {
        ...
        if (error) throw new Error(error.message);
        db.runSync('DELETE FROM outbox WHERE id = ?', op.id);
        // Media cleanup on tombstone-ack was removed with the photo
        // persistence switch-off (Sept 2026): uploads are disabled, so
        // no cloud bytes can be orphaned by a delete.
        result.pushed += 1;
      }
    } catch (e) {
